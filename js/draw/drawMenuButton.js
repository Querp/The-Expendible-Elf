function drawMenuButton(button) {
        ...
        cnv.textSize(25)
        cnv.text(amount, button.x - 150, button.y)

        // stat value
        let statValue;
        cnv.textSize(25)
        if (button.name === 'intern') statValue = Game.upgrades[button.name].amount;
        if (button.name === 'dash') statValue = `${3 - Game.upgrades[button.name].amount * 0.5}s`;
        if (button.name === 'speed') statValue = `${elf.speed + Game.upgrades[button.name].amount}vel`;
        if (button.name === 'health') statValue = `${Game.health + Game.upgrades[button.name].amount * 100}`;
